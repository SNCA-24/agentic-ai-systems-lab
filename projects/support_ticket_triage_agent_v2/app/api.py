...
from app.schemas import (
    ...
    ApprovalRequest,
    ApprovalResponse,
    CheckpointedResumeResponse,
    CheckpointedTriageResponse,
    ResumeResponse,
    TriageRequest,
    TriageResponse,
)
...
@app.post("/tickets/triage/checkpointed", response_model=CheckpointedTriageResponse)
def triage_ticket_checkpointed(request: TriageRequest) -> CheckpointedTriageResponse:
    """
    Run ticket triage through the checkpoint-enabled graph variant.

    This endpoint keeps the normal triage endpoint unchanged while exposing a
    thread_id-based checkpointed graph path for local durable-workflow experiments.
    """
    thread_id = build_thread_id(request.ticket_id)
    initial_state: AgentState = {
        "ticket_id": request.ticket_id,
        "user_message": request.user_message,
        "category": None,
    ...
    config.update(
        {
            "run_name": "support_ticket_triage_checkpointed_api_run",
            "tags": [
                "support-ticket-triage",
            ...
            "metadata": {
                ...
                "thread_id": thread_id,
                "classifier_mode": CLASSIFIER_MODE,
                "run_source": "checkpointed_triage_api",
                "environment": APP_ENV,
                "langsmith_project": LANGSMITH_PROJECT_NAME,
            },
        }
    ...
    result = checkpointed_ticket_graph.invoke(initial_state, config=config)

    return CheckpointedTriageResponse(
        thread_id=thread_id,
        ticket_id=result["ticket_id"],
        category=result["category"],
        intent=result["intent"],
...
@app.post("/tickets/{ticket_id}/resume/checkpointed", response_model=CheckpointedResumeResponse)
def resume_ticket_checkpointed(ticket_id: str) -> CheckpointedResumeResponse:
    """
    Resume a high-risk workflow through the checkpoint-enabled approval resume graph.

    This endpoint keeps the normal resume endpoint unchanged while exposing a
    thread_id-based checkpointed resume path for local durable-workflow experiments.
    """
    record = get_approval_record(ticket_id)
    if record is None:
        raise HTTPException(
    ...
    initial_state: AgentState = {
        "ticket_id": ticket_id,
        "user_message": "Resume checkpointed high-risk workflow after approval decision.",
        "category": "technical",
        "intent": "high_risk_account_or_financial_action",
        "risk_level": "high",
        "needs_human_review": True,
        "confidence": 0.99,
        "decision_summary": "Checkpointed high-risk workflow resume requested after human approval decision.",
        "approval_status": record.approval_status,
        "approval_id": record.approval_id,
        "approval_notes": record.approval_notes,
        "approved_by": record.approved_by,
    ...
    config.update(
        {
            "run_name": "support_ticket_triage_checkpointed_resume_run",
            "tags": [
                "support-ticket-triage",
            ...
            "metadata": {
                ...
                "approval_id": record.approval_id,
                "approved_by": record.approved_by,
                "run_source": "checkpointed_resume_api",
                "environment": APP_ENV,
                "langsmith_project": LANGSMITH_PROJECT_NAME,
            },
        }
    ...
    result = checkpointed_approval_resume_graph.invoke(initial_state, config=config)
    tool_results = result.get("tool_results", [])
    last_tool_result = tool_results[-1] if tool_results else None

    return CheckpointedResumeResponse(
        thread_id=thread_id,
        ticket_id=result["ticket_id"],
        approval_status=result["approval_status"],
        approval_id=result["approval_id"],
